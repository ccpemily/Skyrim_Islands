...
当前仓库已经不再只是脚手架。第一版原型已经能在游戏内实际跑通“任务触发 -> 迁移到空岛 -> 在空岛落地”的完整主链，功能设想仍继续补充在 `FEATURES.md` 中，但代码层面已经进入“可玩原型持续打磨”阶段。
...
- 空岛专用世界云层已接入：
  ...
  - 当前使用独立云层半径偏移与独立世界绘制层
  - 原版世界云层已通过 XML Patch 禁用，避免和空岛云层重叠
  - 当前云层动画仍通过加速 `_GameTime` 近似控制速度
  - 从空岛小地图进入世界、切到地表再返回时，会自动把背景世界视角纠正回空岛层，避免背景云消失
...
- 开局迁移任务原型已落地：
  - 开局发送提示信并创建任务
  - 玩家接受任务后，普通 `PassengerShuttle` 降落到初始殖民地附近
  - 降落动画结束后延迟弹出装载界面
  - 装载界面已改为 `TransferableOneWayWidget` 方案
  - 可合并的同类物品会合并成一行，并可直接调整数量
  - 当前仍按“殖民者 / 机械族 / 动物 / 物品”分段显示
  - 至少需要选择一名殖民者，机械族不能单独迁移
  - 选中的内容以“Thing + Count”形式进入后续迁移流程
  - 迁移前可确认是否遗弃原殖民地
  - 不再让玩家手动选空岛位置，而是在任务触发地附近随机一个可用锚点
  - 过场结束后会切到新空岛地图，再播放抵达穿梭机动画并卸载
- 穿梭机迁移链路已调整：
  - 起飞与到达统一使用普通 `PassengerShuttle`
  - 到达落地动画支持继承旋转
  - 当前朝向已经校正回左右展开
  - 旧殖民地不再调用 `Abandon(false)`，改为手动移除，避免错误统计被遗弃的殖民者数量
...
- 迁移任务目前仍是第一版线性原型，尚未接入完整 QuestScript / QuestNode 编排、正式过场文本与后续分支内容

### 当前最小闭环状态

当前版本已经能跑通一个“可进入、可迁移”的最小空岛闭环：

1. 新游戏开始
2. `WorldComponent_SkyIslands` 运行时注册空岛专属图层
3. `GameComponent_SkyIslandFlow` 发送提示信并创建迁移任务
4. 玩家接受任务，穿梭机降落到初始殖民地附近
5. 玩家通过可合并数量的装载界面选择殖民者、机械族、动物和物资
6. 系统在任务触发地附近自动随机一个可用空岛锚点
7. 通过自定义地图生成器生成空岛地图
8. 当前视角切换到空岛地图
9. 穿梭机抵达并卸载已选中的 Pawn 与物资

这说明“空岛作为可访问地图存在”和“通过任务迁移到空岛”这两件事都已经成立，后续重点已经不是从零搭入口，而是继续打磨空岛层与背景渲染、专属天气、任务内容和后续玩法系统。
...
- 云层独立平移速度
  当前自定义云层已能稳定显示，但“单独控制云层平移速度而不是直接加速动画时间”的需求仍暂缓
...
初始任务最适合由 `GameComponent.StartedNewGame()` 或剧本 `ScenPart_CreateQuest` 驱动。当前第一版原型已经做成“提示信 + 任务 + 降落穿梭机 + 装载界面 + 过场迁移 + 新空岛抵达”的线性流程。后续若要继续正式化，再向完整任务树收束即可。

当前流程可以概括为：

1. 新游戏开始后发送提示信并创建任务
2. 玩家接受任务后，穿梭机降落到初始殖民地附近
3. 降落动画结束后弹出装载界面
4. 玩家选择殖民者、机械族、动物和物资，并确认是否遗弃原殖民地
5. 系统自动在任务触发地附近随机一个空岛锚点
6. 通过长事件生成空岛地图并切镜头
7. 穿梭机抵达空岛并卸载

后续不再建议回到“玩家手动世界选点”的方案，而应保持任务驱动的自动选址，再把空岛移动功能留给后续正式系统。
...
优先把当前已经搭好的“独立 PlanetLayer + 专属 Biome / Weather + 迁移任务原型”继续调到视觉和行为都稳定。

理由是：

- “空岛地图 + 开局迁移任务 + 独立层 + 专属天气”这条主链已经基本连起来了
- 现在最大的工作重心转向：
  - 视觉一致性
  - 层间渲染一致性
  - 云层与天气观感
