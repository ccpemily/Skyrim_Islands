...
- 云海研究 demo：
  ...
  - 研究完成后可在云海上划定钓鱼区
- 空岛移动系统（已完整重写）：
  - 横向 4 档位变速移动（前进一至前进四），支持加速/巡航/减速/泊入/中断状态
  - 纵向高度同步：1 km/h 匀速升降，与横向时间取瓶颈同步完成
  - 动态换挡：加速/巡航段可实时切换档位并重新规划剩余轨迹
  - 路径规划与总控界面，含小地图、档位横条、路线摘要与状态显示
  - 档位即启动：空闲状态下点击非停止档位可直接按该档位启动引擎；无路线时提示拒绝
